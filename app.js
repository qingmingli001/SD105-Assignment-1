...
function searchViaStreetKey(streetKey){
  ...
  .then(data =>{
    console.log(data);
    data.stops.forEach(item => getStopInformation(item));
    console.log(stopInformation);
    return stopInformation;
  }).then(data=>{
    console.log(data);
    data.forEach(item=>{
      addRouteInformation(item.stop_key);
    });
...
function addRouteInformation(stop_key){
  ...
  .then(data =>{
    ...
    data['stop-schedule']['route-schedules'].forEach(item => {
      ...
      const iterationNumber = item['scheduled-stops'].length > 2 ? 2 : item['scheduled-stops'].length;
...
document.addEventListener('click',(event)=>{
  if (event.target.nodeName==="A") {
    ...
    createTable(detailInformation);
  }
})

function getStopInformation(objectArg){
const tableData = {};
tableData.stop_key = objectArg.key;
stopInformation.push(tableData);
}
